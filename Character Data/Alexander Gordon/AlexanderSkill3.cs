...
public class AlexanderSkill3 : MonoBehaviour
{
    ...
    public float damageMultiplierPerTick = 2.5f;
    ...
    private IEnumerator UseSkill3Coroutine()
    {
        ...
        foreach (Collider2D enemy in hitEnemies)
        {
            ...
            if (enemyStat != null)
            {
                int damage = Mathf.CeilToInt(playerCharacter.SkillPower.BaseValue * damageMultiplier);
        ...
        for (int i = 0; i < damageTicks; i++)
        {
            ...
            hitEnemies = Physics2D.OverlapCircleAll(transform.position, skillRange);
            ...
            foreach (Collider2D enemy in hitEnemies)
            {
                ...
                if (enemyStat != null)
                {
                    int damage = Mathf.CeilToInt(playerCharacter.SkillPower.BaseValue * damageMultiplierPerTick);
                    enemyStat.TakeDamage(damage);
                }
            }
        }
